import numpy as np
def findNum(encoded) -> int:
    form = ('one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine')
#         receipts = ('1abc2nine83mpnsixfcmxcbnspx',
# 'pqr3stu8vwxfourth',
# 'a1b2c3d4e5oneight',
# 'treb7uchet0two')
    breaking = False
    ...
    if num == 0:
        for i in (range(len(encoded))):
            ...
            if ord(encoded[i]) >= 48 and ord(encoded[i]) <= 57:
                index1 = i
                break   
        
        for i in range(3, len(encoded) + 1):
            for j in range(len(form)):
                if form[j]  in encoded[:i]:
                    index2 = i - len(form[j])
                    integer = j + 1
                    breaking = True
                    break
    ...
    if index1 <= index2 and index1 != -1:
        num = int(encoded[index1])
    elif index2 < index1 and index2 != -1:
        num = integer
    ...
    if num != 0:
        for i in range(1, len(encoded) + 1):
            ...
            if ord(encoded[-i]) >= 48 and ord(encoded[-i]) <= 57:
                index1 = len(encoded) - i
                break

        for i in range(3, len(encoded)):
            for j in range(len(form)):
                if form[j]  in encoded[-i:len(encoded)]:
                    index2 = len(encoded) - i
                    integer = j + 1
                    breaking = True
                    break
            if breaking:
    ...
    if index1 >= index2 and index2 != -1:
            num*= 10
            num += int(encoded[index1])
    elif index1 < index2 and index1 != -1:
            num*=10
            num += integer
...
def main():
    secret = []
    receipts = ('tmmnhlxzpj1eightldxhjnone97',
...
# 'treb7uchet0two')
    for receipt in receipts:
        print(findNum(receipt))
        secret.append(findNum(receipt))
